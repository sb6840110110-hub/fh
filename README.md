...
<head>
    ...
    <title>ClassCraft Studio - ระบบห้องเรียนออนไลน์</title>
...
<body class="bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 min-h-screen text-gray-800">
        ...
        <div class="glass bg-white rounded-3xl p-6 w-full max-w-4xl shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div id="teacherControlBox" class="bg-indigo-50/70 p-4 rounded-2xl border border-indigo-100 mb-4 flex flex-col md:flex-row justify-between items-center gap-3">
                ...
                <div class="flex space-x-2 flex-wrap gap-y-2">
                    ...
                    <!-- ปุ่มเปิดตารางคะแนนรวม (เฉพาะครู) -->
                    <button onclick="openGradebookModal()" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-2 rounded-xl text-sm font-medium shadow transition">
                        📊 ตารางคะแนนรวม
                    </button>
    ...
    <!-- Modal: ตารางสรุปคะแนนรวมทั้งหมด (เฉพาะคุณครู) -->
    <div id="gradebookModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden z-50 flex items-center justify-center p-4">
        <div class="glass bg-white rounded-3xl p-6 w-full max-w-5xl shadow-2xl flex flex-col max-h-[85vh]">
            <div class="flex justify-between items-center border-b pb-3 mb-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-800">📊 ตารางสรุปคะแนนรวมนักเรียน</h2>
                    <p class="text-xs text-indigo-600 mt-0.5">🔒 สิทธิ์การเข้าถึงเฉพาะคุณครูผู้สอนเท่านั้น</p>
                </div>
                <button onclick="closeGradebookModal()" class="text-gray-400 hover:text-gray-600 font-bold text-xl">✕</button>
            </div>

            <div class="overflow-x-auto overflow-y-auto flex-1 rounded-2xl border border-gray-200 shadow-inner">
                <table class="w-full text-left border-collapse bg-white">
                    <thead id="gradebookThead" class="bg-indigo-50 text-indigo-900 text-xs sticky top-0 border-b border-indigo-100">
                        <!-- หัวตารางจะถูกเติมด้วย JavaScript -->
                    </thead>
                    <tbody id="gradebookTbody" class="text-xs divide-y divide-gray-100">
                        <!-- รายชื่อคะแนนนักเรียนจะถูกเติมด้วย JavaScript -->
                    </tbody>
                </table>
            </div>

            <div class="flex justify-between items-center mt-4 pt-3 border-t">
                <span class="text-xs text-gray-500">* คะแนนจะอัปเดตให้อัตโนมัติเมื่อมีการตรวจงานและบันทึกคะแนน</span>
                <button onclick="closeGradebookModal()" class="px-5 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-xl text-sm font-medium">ปิดหน้าต่าง</button>
            </div>
        </div>
    </div>

    <!-- Modal: ลงทะเบียนเข้าห้องเรียน -->
        ...
        <div class="glass bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div class="bg-indigo-50/60 p-4 rounded-2xl border border-indigo-100 mb-4 space-y-3">
                <div class="flex justify-between items-center text-xs">
                    ...
                    <button onclick="openPreviewWorkForTeacher()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-2.5 py-1 rounded-lg font-medium shadow">🔍 เปิดพรีวิวไฟล์</button>
                </div>
                <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">ให้คะแนน (ตัวเลข เช่น 10)</label>
                        <input type="number" step="any" id="inputGradeScore" class="w-full px-3 py-2 text-sm bg-white border rounded-xl outline-none font-bold text-indigo-600" placeholder="เช่น 10">
                    </div>
                </div>
    ...
    <!-- Modal: สร้างห้องเรียนใหม่ (สำหรับครู) -->
    ...
    <script>
        ...
        let classStudents = {};
        let classAssignments = {}; // เก็บรายการสั่งงานของแต่ละห้อง
        let submissions = {};
        let streamPosts = {};
        ...
        function unsubmitAssignment(assignId) {
            if (confirm('คุณต้องการยกเลิกการส่งงานใช่หรือไม่?')) {
                if (submissions[activeRoomCode] && submissions[activeRoomCode][assignId]) {
                    delete submissions[activeRoomCode][assignId][currentStudentName];
                    updateStudentSubmitUI();
                    addNotification(`↩️ คุณได้ยกเลิกการส่งงานในวิชา [${activeRoomCode}] แล้ว`);
                    alert('ยกเลิกการส่งงานเรียบร้อยแล้ว');
                }
            }
        }

        function previewMySubmission(assignId) {
            const sub = submissions[activeRoomCode]?.[assignId]?.[currentStudentName];
            if (!sub) return;
            ...
            if (sub.fileData) {
                ...
                container.innerHTML = `
                    <div class="text-center p-8 text-gray-500">
                        ...
                        <p class="font-bold text-sm">ตัวอย่างไฟล์งาน (${sub.fileName || 'document.pdf'})</p>
                        <p class="text-xs text-gray-400 mt-1">ไฟล์เอกสารประกอบการส่งงานของนักเรียน</p>
                    </div>
                `;
            }
        ...
        function updateStudentSubmitUI() {
            ...
            document.querySelectorAll('.assignment-card').forEach(card => {
                ...
                const uploadBox = document.getElementById(`uploadBox-${assignId}`);
                ...
                if (subData) {
                    ...
                    if (uploadBox) {
                        uploadBox.innerHTML = `
                            <div class="bg-indigo-50/50 border border-indigo-100 p-2 rounded-xl mb-2 flex justify-between items-center text-xs">
                                <span class="font-bold text-indigo-900 truncate max-w-[150px]">📁 ${subData.fileName}</span>
                                <button onclick="previewMySubmission('${assignId}')" class="text-indigo-600 hover:text-indigo-800 font-bold bg-white px-2 py-1 rounded-lg border shadow-sm">
                                    🔍 ดูไฟล์
                                </button>
                            </div>
                        `;
                    ...
                    if (actionBtnBox) {
                        actionBtnBox.innerHTML = `
                            <button onclick="unsubmitAssignment('${assignId}')" class="w-full bg-red-100 hover:bg-red-200 text-red-700 py-1.5 rounded-lg text-xs font-bold transition">
                                ↩️ ยกเลิกการส่งงาน (Unsubmit)
                            </button>
                        `;
                    }
                } else {
                    ...
                    if (sub) {
                        ...
                        actionBtn = `
                            <button onclick="openGradeStudentModal('${student.name}')" class="bg-indigo-50 hover:bg-indigo-100 text-indigo-600 px-2.5 py-1 rounded-lg text-xs font-bold transition">
                                ${sub.grade !== null ? '✏️ แก้ไขคะแนน' : '✍️ ให้คะแนน'}
                            </button>
                        `;
                    }
        ...
        function openGradeStudentModal(studentName) {
            ...
            document.getElementById('inputGradeScore').value = sub.grade !== null ? sub.grade : '';
        ...
        function saveGradeAndFeedback() {
            const scoreVal = document.getElementById('inputGradeScore').value.trim();
            ...
            if (submissions[activeRoomCode] && submissions[activeRoomCode][activeCheckAssignId] && submissions[activeRoomCode][activeCheckAssignId][gradingStudentName]) {
                ...
                sub.grade = scoreVal !== "" ? parseFloat(scoreVal) : null;
                sub.comment = comment;
        ...
        /* ===== ระบบตารางสรุปคะแนนรวม (Gradebook) ===== */
        function openGradebookModal() {
            ...
            const thead = document.getElementById('gradebookThead');
            ...
            const assignments = classAssignments[activeRoomCode] || [];
            const roomSubs = submissions[activeRoomCode] || {};

            // สร้างหัวตาราง (Headers)
            let headerRow = `<tr class="border-b"><th class="p-3">ลำดับ</th><th class="p-3">รหัส</th><th class="p-3">ชื่อ - นามสกุล</th>`;
            assignments.forEach((assign, index) => {
                headerRow += `<th class="p-3 text-center border-l border-indigo-100">${assign.title}</th>`;
            });
            headerRow += `<th class="p-3 text-center border-l-2 border-indigo-200 bg-indigo-100/60 font-bold text-indigo-950">รวมทั้งหมด</th></tr>`;
            thead.innerHTML = headerRow;

            // สร้างแถวข้อมูลของนักเรียนแต่ละคน (Rows)
            if (students.length === 0) {
                tbody.innerHTML = `<tr><td colspan="${assignments.length + 4}" class="p-8 text-center text-gray-400">ยังไม่มีนักเรียนลงทะเบียนในห้องเรียนนี้</td></tr>`;
            } else {
                students.forEach((student, sIdx) => {
                    let totalScore = 0;
                    let row = `<tr class="hover:bg-gray-50/80 transition"><td class="p-3 text-gray-400 font-mono">${sIdx + 1}</td><td class="p-3 font-mono text-gray-500">${student.id}</td><td class="p-3 font-bold text-gray-800">${student.name}</td>`;

                    assignments.forEach(assign => {
                        const sub = roomSubs[assign.id]?.[student.name];
                        let scoreDisplay = `<span class="text-gray-300">-</span>`;

                        if (sub) {
                            if (sub.grade !== null && sub.grade !== undefined) {
                                scoreDisplay = `<span class="font-bold text-indigo-600">${sub.grade}</span>`;
                                totalScore += parseFloat(sub.grade) || 0;
                            } else {
                                scoreDisplay = `<span class="text-amber-500 text-[11px] font-semibold bg-amber-50 px-2 py-0.5 rounded">รอตรวจ</span>`;
                            }
                        }

                        row += `<th class="p-3 text-center border-l border-gray-100 font-normal">${scoreDisplay}</th>`;
                    });

                    row += `<td class="p-3 text-center border-l-2 border-indigo-100 bg-indigo-50/30 font-bold text-emerald-600 text-sm">${totalScore}</td></tr>`;
                    tbody.innerHTML += row;
                });
            }

            document.getElementById('gradebookModal').classList.remove('hidden');
        }
        ...
        function openPreviewWorkForTeacher() {
            const sub = submissions[activeRoomCode]?.[activeCheckAssignId]?.[gradingStudentName];
            const container = document.getElementById('previewContentContainer');
            document.getElementById('previewFileNameTitle').innerText = sub?.fileName || 'ไฟล์งาน';
        ...
        function addAssignment(e) {
            ...
            if (!classAssignments[activeRoomCode]) classAssignments[activeRoomCode] = [];
            classAssignments[activeRoomCode].push({ id: assignId, title: title });
